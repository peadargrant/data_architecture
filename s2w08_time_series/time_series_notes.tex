...
Assume that we transform the CSV representation using \texttt{curl} to \texttt{covid.csv} using:
\begin{minted}{bash}
./get_covid_data.sh
\end{minted}

Then we simplify the CSV to produce a summary using pandas (to just select certain fields):
\inputminted{python}{summarise_covid_data.py}

Table setup in PostgreSQL:
...
\href{https://www.postgresql.org/docs/13/tutorial-window.html}{Window functions} are like aggregate functions except that the aggregation occurs over a defined \textbf{window frame}, or set of rows relative to the current row, rather than over the whole table.
